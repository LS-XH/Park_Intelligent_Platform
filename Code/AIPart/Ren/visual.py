...
from model import ObstacleGenerator, AgentGroup, MAP_SIZE, num_agents, emergency
...
for i, obs in enumerate(obstacle_gen.obstacles):
    for idx, gap_center in obs.gap_center_map.items():
        ...
        # 计算并绘制斥力线段
        seg_length = obs.gap_offset_ratio + (obstacle_gen.intersection_gap_size / 2) + 2
        dir_vec = np.array([obs.dir_x, obs.dir_y])
