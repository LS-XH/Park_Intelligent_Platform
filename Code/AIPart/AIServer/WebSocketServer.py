...
class WebSocketServer:
    ...
    def on_simulation(self, func):
        """注册模拟数据生成函数的装饰器"""
        @wraps(func)
        async def wrapper(websocket, stop_event, sim_type):
            while not stop_event.is_set():
                data = await func(sim_type)  # 调用被装饰的生成函数
    ...
    async def handle_client(self, websocket):
        ...
        try:
            ...
            async for message in websocket:
                ...
                if status is None:
                    await websocket.send(json.dumps({
                        "status": -1,
                        "response": {"content": "Missing 'status' in message"}
                    }))
                    continue

                # 处理持续传输的启动请求（状态码3/5/7/9/11）
                if status in [3, 5, 7, 9, 11]:
                    sim_type = status
                    # 如果该类型任务已存在，先停止
                    if sim_type in simulation_tasks:
                        existing_stop_event, existing_task = simulation_tasks[sim_type]
                    ...
                    task = asyncio.create_task(
                        self.simulation_handler(websocket, stop_event, sim_type)
                    )
                    simulation_tasks[sim_type] = (stop_event, task)

                    self.logger.info(f"开始向 {client_ip} 发送类型 {sim_type} 的模拟数据")
                    await websocket.send(json.dumps({
                        "status": sim_type,
                        "response": {"content": "started"}
                    }))

                # 处理持续传输的停止请求（状态码4/6/8/10/12）
                elif status in self.stop_to_start_map:
                    # 通过映射关系找到对应的启动类型
                    sim_type = self.stop_to_start_map[status]
                    if sim_type in simulation_tasks:
                        stop_event, task = simulation_tasks[sim_type]
                        stop_event.set()
                        await task
                        del simulation_tasks[sim_type]
                        # 从字典中移除已停止的任务
                        self.logger.info(f"已停止 {client_ip} 的类型 {sim_type} 模拟数据传输")
                ...
                # 处理静态消息（其他状态码）
                    ...
                    if self.message_handler:
                        response = await self.message_handler(message, websocket)
                        await websocket.send(response)
                    else:
                        await websocket.send(json.dumps({
                            "status": -1,
                            "response": {"content": "No message handler registered"}
                        }))
            ...
            # 客户端断开连接时停止所有运行中的模拟任务
            for sim_type, (stop_event, task) in simulation_tasks.items():
                stop_event.set()
                await task
            self.logger.info(f"客户端 {client_ip} 断开连接，所有模拟任务已停止")
